# src/tsc/models/distilbert/predict_distilbert.py
...
from tsc.utils.paths import DISTILBERT, DATA_TWITTER
...
# Tokenize dataset using DistilBERT pretrained tokenizer
...
def predict(model, test_tokenized, batch_size):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    model.to(device)
    model.eval()

    dataloader = DataLoader(test_tokenized, batch_size=batch_size, shuffle=False)
...
def main():
    ...
    tokenizer = AutoTokenizer.from_pretrained(DISTILBERT / "best_model" / "artifacts")
    model = AutoModelForSequenceClassification.from_pretrained(
        DISTILBERT / "best_model" / "artifacts"
    )
    ...
    preds = predict(model, test_tokenized, args.batch_size)
    y_pred = 2 * preds - 1

    create_csv_submission(ids, y_pred, "predictions_distilbert")
